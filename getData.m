...
function [X,A,B,C] = getData(rank, index)
...
%     A(1:I/2,1) = rand(I/2,1);
%     A(5*(I/6)+1: I,1) = rand(I/6, 1);
%     A((I/6 + 1):5*(I/6),2) = rand(2*(I/3),1);
%     A(1:I/2,3) = rand(I/2,1);
%     A(2*(I/3)+1: I,3) = rand(I/3, 1);

    A(1:I/3,1) = rand(I/3,1);
    A((I/3 + 1):2*(I/3),2) = rand(I/3,1);
    A(2*(I/3)+1: I,3) = rand(I/3, 1);

    B =A;

    C(I/6 + 1:I/2,1) = rand(I/3,1);
    C((2*I/3 + 1):I,1) = rand(I/3,1);
    C((I/6) + 1:7*I/12,2) = rand(5*I/12, 1);
    C(5*(I/6)+1:I,2) = rand(I/6, 1);
    C((I/6 + 1):I/2,3) = rand(I/3,1);
    C((2*I/3 + 1):I,3) = rand(I/3,1);
...
if(R==4)
    ...
    B(I/5+1:I/2,3)=rand(3*I/10,1);
    B(7*I/10+1:I,3)=rand(3*I/10,1);
    B(1:I/4,4)=rand(I/4,1);
    B((I/2)+1:(3*I/4),4)=rand(I/4,1);
    C(1:K/4,1)=rand(K/4,1);
    C((K/4)+1:3*K/4,[2 3])=rand(K/2,2);
    C((3*K/4)+1:K,4)=rand(K/4,1);
%     A(A>0) = 1;
%     B(B>0) = 1;
%     C(C>0) = 1;
%     X=sptensor(reshape(khatrirao(B,A)*C',[I J K]));
%     X=ktensor([A B C], ones(R,1));
end
if(R==5)
